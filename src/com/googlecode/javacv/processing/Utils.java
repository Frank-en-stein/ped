...
public class Utils {
	 ...
	 * @param src : IplImage iplImg = cvCreateImage(size, IPL_DEPTH_8U, 3);
	 * @param dst
	 * @param RGB : RGB -> true ou BGR ->false
	 */
	static public void PImageToIplImage(PImage src, IplImage dst, boolean RGB) {

		src.loadPixels();
		if (RGB) {
			for (int i = 0; i < src.width * src.height; i++)
				cvSet1D(dst, i, CV_RGB((double) ((src.pixels[i] & 0xFF0000)>> 16), 
						(double)((src.pixels[i] & 0xFF00) >> 8), 
						(double)(src.pixels[i] & 0xFF)));

		} else {
			for (int i = 0; i < src.width * src.height; i++) 
				cvSet1D(dst, i, CV_RGB((double) (src.pixels[i] & 0xFF0000), 
						(double)(src.pixels[i] >> 8 & 0xFF00), 
						(double)(src.pixels[i] >> 16 & 0xFF)));
