...
public class FeatureExtractor extends PApplet {
	...
	public void setup() {
		...
		imgShowWidth = ((screenWidth/2-100<imgsWidth)?(((screenWidth)/2)-100):(imgsWidth/2)-100);
	...
	public void compute(int theValue) {
		if(resultCaching[imgDID][imgGID]==null){
			...
			Utils.PImageToIplImage(imgG, ipl1, true);
			IplImage ipl2 = cvCreateImage(size, IPL_DEPTH_8U, 3);
			Utils.PImageToIplImage(imgD, ipl2, true);
			...
			sauvola.filter(ipl1, ipl3);
			sauvola.filter(ipl2, ipl1);
			
			cvSub(ipl1, ipl3, ipl2, null);*/
		...
		cvSmooth(ipl5, ipl5, CV_MEDIAN, 7);
